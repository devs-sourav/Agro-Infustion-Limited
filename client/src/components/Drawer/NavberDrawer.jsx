...
const NavberDrawer = ({ menulist, isShopDrawerOpen, toggleDrawer }) => {
  return (
    <>
      ...
      >
        <div className="h-screen md:w-80 bg-white z-50  transition-transform duration-300 ease-in-out lg:hidden">
          <div
            onClick={toggleDrawer}
            className="flex justify-between items-center px-4 my-12"
          >
            <img className="w-[50px] h-[50px] cursor-pointer" src={logo} alt="Logo" />
            <RxCross2 className="w-7 h-7 p-1 font-extrabold bg-primary cursor-pointer rounded text-white" />
          </div>
          <ul className="my-8 space-y-4 px-4">
            {menulist?.map(
              (item, index) =>
